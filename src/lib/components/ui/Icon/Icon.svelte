<script lang="ts">
  export let name: string;
  export let width: string = "24";
  export let height: string = "24";
  export let alt: string = "";
  export let className: string = "";

  let iconPath = `/icons/${name}.svg`;
  
  // Inline SVG fallback for common icons
  const inlineIcons: Record<string, string> = {
    email: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
    </svg>`,
    lock: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
    </svg>`,
    user: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
    </svg>`
  };
  
  $: useInline = inlineIcons[name];
</script>

{#if useInline}
  <span class={className} style="width: {width}px; height: {height}px; display: inline-flex; align-items: center; justify-content: center;">
    {@html useInline}
  </span>
{:else}
  <img src={iconPath} alt={alt || name} {width} {height} class={className} />
{/if}
