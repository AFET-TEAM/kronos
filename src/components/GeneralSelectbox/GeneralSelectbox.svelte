...
<div class="selectbox-wrapper" bind:this={rootEl}>
  {#if label}
    <label class="selectbox-label">{label}</label>
  {/if}
  <div class="selectbox-container" tabindex="0">
    {#if leftIcon}
      <img src={leftIcon} alt="left icon" class="selectbox-icon left" />
    {/if}
    <div class="selectbox-value" on:click={toggleDropdown}>
      {#if selected}
    ...
    <button type="button" class="selectbox-icon right selectbox-arrow-btn" on:click={toggleDropdown} tabindex="-1" aria-label="Açılır menüyü göster">
